...
// Wrap the main content in Suspense to handle searchParams in Client Components safely
...
function LoginForm() {
  ...
  // Check for NextAuth errors in URL (e.g., ?error=CredentialsSignin)
  const authError = searchParams.get('error');

  // ---------------------------------------------------------
  // 1. Social Login Logic (Manual Redirects)
  // ---------------------------------------------------------
  const handleSocialLogin = (provider: 'kakao' | 'google') => {
    setIsLoading(true);

    // Exact URLs we discussed previously
  ...
  // ---------------------------------------------------------
  // 2. Credentials Login Logic (Email/Password)
  // ---------------------------------------------------------
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Calls the 'credentials-login' provider in your auth.ts
      const result = await signIn('credentials-login', {
        email,
        password,
        redirect: false, // We handle redirect manually to show errors nicely
      });
